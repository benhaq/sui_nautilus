module medical_vault::seal_whitelist {
    ...
    const E_INVALID_ENCLAVE_SIGNATURE: u64 = 7;

    /// Intent for enclave public key registration
    const ENCLAVE_PK_INTENT: u8 = 1;
    ...
    /// Signing payload struct signed by the enclave keypair for public key registration
    public struct EnclavePK has copy, drop {
        pk: vector<u8>,
    }
    ...
    /// Get reference to whitelist UID (for dynamic fields)
    public fun uid_mut(whitelist: &mut SealWhitelist): &mut UID {
        &mut whitelist.id
    }

    /// Get reference to whitelist UID (for dynamic fields)
    public fun uid(whitelist: &SealWhitelist): &UID {
        &whitelist.id
    }
    ...
    ////////////////////////////////////////////////////////////
    // Seal approve for enclave-based whitelist management
    ////////////////////////////////////////////////////////////

    /// Helper function to check the address derived from a public key.
    /// Assumes ed25519 flag for enclave's ephemeral key and a Sui address is derived as
    /// blake2b_hash(flag || pk).
    ...
    /// Seal approve policy for enclave-based whitelist management.
    ...
    ///
    /// This enables enclaves to manage whitelist access programmatically with cryptographic
    /// guarantees that only authorized enclave instances can modify whitelist policies.
    entry fun seal_approve_enclaves(
        id: vector<u8>,
        signature: vector<u8>,
        wallet_pk: vector<u8>,
        timestamp: u64,
        ...
        assert!(ctx.sender().to_bytes() == pk_to_address(&wallet_pk), E_INVALID_ENCLAVE_SIGNATURE);
        ...
        assert!(ed25519::ed25519_verify(&signature, enclave.pk(), &payload), E_INVALID_ENCLAVE_SIGNATURE);
    }
}
